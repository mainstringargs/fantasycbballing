...
contests = draft_kings.Client().contests(sport=draft_kings.Sport.COLLEGE_BASKETBALL)
...
for contest in contests.contests:
    ...
    if 'showdown' in contest.name.lower() and contest.entries_details.fee < 2 and 'top 20' not in contest.name.lower() and 'satellite' not in contest.name.lower() and 'winner takes all' not in contest.name.lower() and weekday.lower() in \
        ...
        dk_df.to_csv(
            ...
            index=False);
        merged_dk_df = dk_df.merge(cbb_projections, on='Name', how='left')
        ...
        try:
            ...
            # export to csv
            combined_csv.to_csv("results/cbb_combined_results_" + teams + "_" + LOGDATE + "_" + now + "_" + str(
